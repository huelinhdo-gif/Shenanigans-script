...
Title.Text = "Shenanigans | Ultimate v3"
...
local MainTabBtn = CreateTabBtn("Misc / Fly")
...
local MainScroll = CreateScrollList()
...
local function SwitchTab(activeBtn, activeScroll)
    ...
    MainTabBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 28)
...
MainTabBtn.MouseButton1Click:Connect(function() SwitchTab(MainTabBtn, MainScroll) end)

-- GLOBAL VARIABLES
local TargetModel, LockOnTargetModel = nil, nil
local ActiveTeleportBtn, ActiveLockBtn = nil, nil
...
local function UpdateESP()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            local highlight = p.Character:FindFirstChild("ShenESP")
            if ESP_Enabled then
                if not highlight then
                    highlight = Instance.new("Highlight")
                    highlight.Name = "ShenESP"
                    highlight.FillColor = Color3.fromRGB(255, 0, 80)
                    highlight.OutlineColor = Color3.fromRGB(255, 255, 255)
                    highlight.Parent = p.Character
                end
            else
                if highlight me:Destroy() end
            end
        end
    end
end
...
local function CreateCard(scroll, textName, modelObj, isLockTab)
    ...
    btn.BackgroundColor3 = isON and Color3.fromRGB(0, 180, 70) or Color3.fromRGB(200, 40, 40)

    btn.MouseButton1Click:Connect(function()
        if isLockTab then
            if LockOnTargetModel == modelObj then StopLockOn() else StopLockOn(); LockOnTargetModel = modelObj; ActiveLockBtn = btn; btn.Text = "ON"; btn.BackgroundColor3 = Color3.fromRGB(0, 180, 70) end
        else
            if TargetModel == modelObj then StopTeleport() else StopTeleport(); TargetModel = modelObj; ActiveTeleportBtn = btn; btn.Text = "ON"; btn.BackgroundColor3 = Color3.fromRGB(0, 180, 70) end
        end
    end)
end

-- TOGGLE BUTTON CREATOR FOR MISC
local function CreateToggle(scroll, titleText, callback)
    local card = Instance.new("Frame")
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.BackgroundColor3 = state and Color3.fromRGB(0, 180, 70) or Color3.fromRGB(200, 40, 40)
        callback(state)
    end)
end

-- SETUP MISC TAB
CreateToggle(MainScroll, "Enable Fly (Bay)", function(state)
    Flying = state
    if Flying then StartFly() else StopFly() end
end)

CreateToggle(MainScroll, "Noclip (Xuyên tường)", function(state) Noclip = state end)
CreateToggle(MainScroll, "ESP Player (Nhìn Xuyên Tường)", function(state) ESP_Enabled = state; UpdateESP() end)
CreateToggle(MainScroll, "Inf Jump (Nhảy Vô Hạn)", function(state) InfJump = state end)
...
UserInputService.InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.X then ToggleGUI() end
end)
